...
- **Fast, typed stack**: **Go** HTTP API (**Chi** on the standard library) plus a **React 19** SPA built with **Vite**, **TypeScript**, and **Tailwind CSS v4**.
- **Data layer**: **PostgreSQL** (users, courses, and relational data via versioned SQL migrations in `server/migrations/`); **MongoDB** is wired in for future document-style workloads.
