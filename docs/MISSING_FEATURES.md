...
| 2.2  | **Question types** — limited to MC / T-F / short answer + AI grading. Missing: matching, ordering, drag-drop, hotspot/image-region, fill-in-the-blank with multiple correct, numeric with tolerance & units, formula (LaTeX) input, code execution, file upload, audio/video response. | MAJOR    | K12, HE, SL | THIN    |
| 2.3  | **Math rendering & input** — no MathJax/KaTeX in [TipTap editor](clients/web/src/components/editor) or a math input keyboard for students. Blocks STEM courses.                                                                                                                        | MAJOR    | K12, HE     | MISSING |
