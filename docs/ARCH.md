...
- **Problem**: Frontend DTOs are hand-maintained in `clients/web/src/lib/*-api.ts`. Backend only documents `/health` in the OpenAPI spec. Every backend change risks silent frontend drift. There is no CI guard.
- **Recommendation**:
  1. Annotate every chi route handler with OpenAPI comments (using `swaggo/swag` or equivalent). Enforce via a CI check that fails when a route in `server/internal/httpserver/` is not referenced in the OpenAPI spec.
...
- **Problem**: Course routes are spread across many files in `server/internal/httpserver/course_*.go` but the largest individual handlers remain long. On the frontend, `course-module-quiz-page.tsx` (2,752), `course-modules.tsx` (2,214), and `quiz-student-take-panel.tsx` (1,969) mix data fetching, business logic, and rendering. These files are the biggest barrier to onboarding and parallel work.
- **Recommendation**:
  - **Backend**: Keep the per-domain file split already in place (`course_get.go`, `course_put.go`, `module_quiz.go`, etc.). Apply the same pattern to any handler file > 500 LOC.
  ...
  - Add an ESLint rule (`max-lines`) set to 500 for `.tsx` and 800 for handler `.go` files, with a grandfather list that must shrink over time.
- **Scope**: 2 eng, 3–4 weeks. Can be done file-by-file without big-bang refactor.
- **Success**: No single file > 1,000 LOC in `server/internal/httpserver/` or `clients/web/src/pages`. Grandfather list empty.
...
- **Recommendation**:
  1. Implement a chi middleware `AuthedUser` that verifies JWT and loads role context once per request, storing it in the request context.
  2. Implement a declarative `RequirePermission("scope:area:fn:action")` middleware factory that runs RBAC checks at the router level.
  3. Introduce a `ResourceScope` pattern for ownership checks: `OwnsCourse(courseID)`, `OwnsSubmission(submissionID)`, etc. Each resolves the resource and asserts the current user has the right relationship.
  4. Add a CI check (simple grep) that flags handlers which read the auth context but declare no permission middleware in the route definition.
- **Scope**: 1 eng, 2 weeks for the middleware work; incremental migration per route file.
- **Success**: Zero manual `require_permission` calls in handler bodies; all auth logic surfaces in the router definition.
...
- **Problem**: Hundreds of `pgx` query calls across repos with inconsistent batching. No visibility into slow queries. Schema has 100+ migrations but no introspection of index coverage. `quiz_attempts` and `submissions` will be the hottest tables; risk is real once a single course has 500+ students.
- **Recommendation**:
  1. Turn on pgx query logging in a staging environment and run the top 10 user flows (course load, quiz take, gradebook view, module nav). Identify queries that run > N times per request.
  2. Introduce a bulk-fetch pattern: replace `for _, id := range ids { repo.Get(ctx, id) }` with `repo.GetMany(ctx, ids)` using `WHERE id = ANY($1)`.
  3. Add a migration covering: `course_enrollments(user_id, course_id)`, `quiz_attempts(user_id, quiz_id, status)`, `submissions(assignment_id, user_id)`, `analytics_events(course_id, created_at DESC)`. Verify with `EXPLAIN ANALYZE`.
  4. Add a `pg_stat_statements` dashboard to the repo's ops docs.
- **Scope**: 1 eng, 2 weeks.
...
- **Problem**: Background work is two periodic goroutines in `server/internal/background/` (quiz auto-submit every 30s, grade release every 30s). Adding anything more complex (LTI outcome sync, email send, PDF generation, course imports) will require a real queue.
- **Recommendation**: Adopt **PostgreSQL-backed job queues** (`pgmq` or a small custom `jobs` table with `FOR UPDATE SKIP LOCKED`). Rationale: no new infra, transactional job enqueue alongside business writes. Reserve Redis for P2 if throughput demands it. Define a `Job` interface with `Handle()` and `RetryPolicy()`; move existing sweepers into it.
- **Scope**: 1 eng, 2 weeks.
- **Success**: No ad-hoc long-running goroutine loops; emails, imports, PDF export all routed through the queue with retry + DLQ.

### P1.2 — Structured Logging and Metrics
- **Problem**: `log/slog` is in use but text-formatted. No request IDs, no metrics, no error aggregation. Debugging a production issue today means SSH + `grep`.
- **Recommendation**:
  1. Switch `slog` to JSON formatting in non-dev builds. Include `request_id`, `user_id`, `course_id` on every log line via a chi middleware that injects these into the request context.
  2. Add a Prometheus middleware for chi; expose `/metrics`. Track: request duration histogram by route, DB query duration, job queue depth/lag, active WebSocket connections.
...
- **Recommendation**: Introduce a `FileStore` interface with `Put`, `Get`, `PresignedURL`, `Delete`. Provide two implementations: `LocalFileStore` (dev) and `S3FileStore` (prod; works with S3, R2, MinIO, GCS). Migrate uploads/imports through the interface. Serve large downloads via presigned URLs, not through the API.
- **Scope**: 1 eng, 2 weeks.
...
- **Recommendation**: Decide now between **shared-schema with `tenant_id` column** (preferred — easier ops, cheaper at <1K tenants) vs. **schema-per-tenant**. Write an ADR. Add `tenant_id UUID NOT NULL` to all new tables. Add a request-scoped `TenantContext` chi middleware. Plan a migration for existing tables (backfill a `default` tenant) before adding the first paying customer.
...
- **Problem**: Two Go broadcast channels (`comm_events`, `feed_events`) exist in `server/internal/commevents/` but no WebSocket endpoints consume them. Frontend polls. Plan 6.x (communication) and 13.x (live features) depend on real-time.
- **Recommendation**: Add a chi WebSocket endpoint `/ws` (using `coder/websocket`) with JWT auth on connect. Bridge broadcast channels to per-connection subscriptions. On the frontend, add a `useLiveQuery(key)` hook that invalidates React Query caches on events.
- **Scope**: 1 eng, 3 weeks.
...
- **Recommendation**: Add Redis as an optional dependency (Compose profile). Introduce a `Cache` interface with `NoopCache` (default), `InMemoryCache`, and `RedisCache` implementations. Use it for: course outline, enrollments, RBAC role lookup, LTI JWKS. Invalidate on writes via tagged keys.
- **Scope**: 1 eng, 2 weeks.
...
- Dependency audit in CI (`govulncheck`, `npm audit`) — fail on High/Critical.
...
- `make` targets for the 10 most common dev tasks (already partially in `server/Makefile`).
- A `CONTRIBUTING.md` describing the four-layer backend split (`httpserver` → `service` → `repos` → `models`) and when to add each.
