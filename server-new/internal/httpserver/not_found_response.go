...
func routerNotFoundHandler(w http.ResponseWriter, r *http.Request) {
	...
	msg := "No HTTP route is registered in the Go server for this path. " +
		"Settings account/AI/system-prompts and most course/assignment surfaces still live in the legacy Rust `server` until ported (e.g. server/src/routes/settings.rs). " +
		"See server-new/migration.md."
	httpserverSlog().Warn("httpserver: 404 (no Go route: chi has no registered handler; request did not match any r.Get/Post/...)",
		...
		slog.String("why", "unported route or different path; /health and registered /api/* paths in server-new return 200/401, not this"),
	)
