  ...
  return (
    <div className="mt-6 space-y-3">
        ...
        >
          <thead>
            <tr className="border-b border-slate-200 bg-slate-50">
              <th
                scope="col"
                className={`sticky top-0 left-0 z-30 ${CELL_PAD} border-b border-r border-slate-200 bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500`}
              >
                Student
              </th>
              {assignments.map((a) => (
                <th
                  key={a.id}
                  scope="col"
                  className={`sticky top-0 z-20 ${CELL_PAD} min-w-[9rem] border-b border-slate-200 bg-slate-50 text-xs font-semibold text-slate-800`}
                >
                  <div className="flex flex-col gap-0.5">
                    <span>{a.name}</span>
                    <span className="font-normal text-slate-500">Out of {a.maxPoints}</span>
                  </div>
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {students.map((student, row) => (
              <tr key={student.id} className="border-b border-slate-100 last:border-b-0">
                <th
                  scope="row"
                  className={`sticky left-0 z-10 ${CELL_PAD} border-r border-slate-200 bg-[#F8F9FA] text-left font-medium text-slate-900`}
                >
                  {student.name}
                </th>
                {assignments.map((a, col) => {
                  ...
                  return (
                    <td
                      ...
                      className={`${CELL_PAD} min-w-[5.5rem] border-l border-slate-100 text-right tabular-nums outline-none transition ${
                        isActive || isEditing
                          ? 'relative z-[1] bg-indigo-50 ring-2 ring-inset ring-indigo-500'
                          : 'bg-white'
                      }`}
                    ...
                    >
                      {isEditing ? (
                        <input
                          ...
                          className="w-full min-w-0 rounded border border-indigo-300 bg-white px-2 py-1 text-right text-sm text-slate-900 tabular-nums shadow-sm outline-none focus:border-indigo-500"
                          value={draft}
                        ...
                        <span className={val ? 'text-slate-900' : 'text-slate-400'}>
                          {val || '—'}
                        </span>
                      )}
                    </td>
      ...
      <p className="text-xs text-slate-500">
        <span className="font-medium text-slate-600">Shortcuts:</span> arrow keys move the cell;
