...
export function CourseCalendar({ courseCode, assignments }: CourseCalendarProps) {
    ...
    return (
      <li key={a.id}>
        <Link
          ...
          className="group flex flex-col gap-0.5 rounded-xl border border-slate-200/90 bg-white px-4 py-3 shadow-sm transition hover:border-indigo-200 hover:bg-indigo-50/40"
        >
          <span className="text-sm font-semibold tracking-tight text-slate-900 group-hover:text-indigo-800">
            {a.title || 'Untitled'}
          </span>
          <span className="text-xs text-slate-500">{formatDueShort(a.dueAt)}</span>
        </Link>
      </li>
    )
  }
  ...
  return (
    <div className="mt-8 space-y-6">
      <div
        className="flex flex-wrap gap-2 rounded-2xl border border-slate-200/90 bg-slate-50/80 p-1.5"
        role="tablist"
      ...
      {view === 'month' && (
        <div className="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm md:p-6">
          ...
          <div className="grid grid-cols-7 gap-px rounded-xl bg-slate-200/90">
            {WEEKDAY_LABELS.map((w) => (
              <div
                key={w}
                className="bg-slate-50 px-1 py-2 text-center text-xs font-semibold uppercase tracking-wide text-slate-500"
              >
                {w}
              </div>
            ))}
            {monthCells.map((cell) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[4.5rem] bg-white p-1.5 text-left ${
                    inMonth ? '' : 'bg-slate-50/90 text-slate-400'
                  }`}
                >
                  <div className="flex items-start justify-between gap-1">
                    ...
                    {n > 0 ? (
                      <span
                        className="inline-flex min-w-[1.25rem] items-center justify-center rounded-full bg-indigo-100 px-1.5 py-0.5 text-xs font-bold text-indigo-800"
      ...
      {view === 'week' && (
        <div className="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm md:p-6">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold tracking-tight text-slate-900">Week view</h2>
      ...
      {view === 'todo' && (
        <div className="rounded-2xl border border-slate-200/90 bg-white p-4 shadow-sm md:p-6">
          <h2 className="text-lg font-semibold tracking-tight text-slate-900">Due today & next 24 hours</h2>
          <p className="mt-1 text-sm text-slate-500">
            Assignments due on today&apos;s date, or with a due time in the next 24 hours.
          </p>
          {todoItems.length === 0 ? (
            <p className="mt-4 text-sm text-slate-500">Nothing in this window.</p>
          ) : (
