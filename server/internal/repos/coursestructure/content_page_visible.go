...
// ContentPageVisibleToStudent mirrors quiz/assignment visibility: published page, published module, module visible_from, relative schedule shift.
func ContentPageVisibleToStudent(
	ctx context.Context, pool *pgxpool.Pool, courseID, pageItemID, userID uuid.UUID, now time.Time,
) (bool, error) {
	var (
	...
	`, pageItemID, courseID, userID).Scan(
