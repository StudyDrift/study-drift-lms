...
func (d Deps) contentPageMarkupsRequireAccess(w http.ResponseWriter, r *http.Request, itemID uuid.UUID) (courseID uuid.UUID, viewer uuid.UUID, ok bool) {
	...
	if !canEdit {
		visible, err := coursestructure.ContentPageVisibleToStudent(r.Context(), d.Pool, *cid, itemID, viewer, time.Now().UTC())
		if err != nil {
...
func (d Deps) handleListContentPageMarkups() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		cid, viewer, ok := d.contentPageMarkupsRequireAccess(w, r, itemID)
		if !ok {
			return
		}
...
func (d Deps) handleCreateContentPageMarkup() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		row, err := markuprepo.Insert(r.Context(), d.Pool, viewer, cid, itemID, "content_page", req.Kind, req.QuoteText, req.NotebookPageID, req.CommentText)
		if err != nil {
