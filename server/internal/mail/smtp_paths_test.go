...
func TestSendPasswordReset_NoSMTP(t *testing.T) {
	if err := SendPasswordResetEmail(config.Config{}, "u@example.com", "http://x", nil); err != nil {
		t.Fatal(err)
	}
}

func TestSendPasswordReset_NoToEmail(t *testing.T) {
	if err := SendPasswordResetEmail(config.Config{}, "", "http://x", nil); err == nil {
...
func TestSendPasswordReset_MissingFrom(t *testing.T) {
	...
	if err := SendPasswordResetEmail(c, "u@x.com", "http://x", nil); err == nil {
		t.Fatal("expected error")
	}
}
...
func TestSendPasswordReset_HostUnreachable_NoAuth(t *testing.T) {
	...
	if err := SendPasswordResetEmail(c, "to@x.com", "http://link", nil); err == nil {
